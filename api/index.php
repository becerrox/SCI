...
$app->delete("/personal/{id:[0-9]+}",function($id) use ($app,$personalService) {
    $data = $personalService->eliminar($id);
    response($app,$data['mensaje'],$data['status']);
});
...
$app->get("/cargos",function() use ($app,$cargosService) {
    $query = $app->request->getQuery();
    unset($query["_url"]);
    $data = $cargosService->listar($query);
    response($app,$data['mensaje'],$data['status']);
});

$app->post("/cargos",function() use ($app,$cargosService) {
    $cargos = json_decode($app->request->getRawBody());
    $data = $cargosService->nuevo($cargos);
    response($app,$data['mensaje'],$data['status']);
});

$app->put("/cargos/{id:[0-9]+}",function($id) use ($app,$cargosService) {
    $cargos = json_decode($app->request->getRawBody());
    $data = $cargosService->modificar($id,$cargos);
    response($app,$data['mensaje'],$data['status']);
});

$app->delete("/cargos/{id:[0-9]+}",function($id) use ($app,$cargosService) {
    $data = $cargosService->eliminar($id);
    response($app,$data['mensaje'],$data['status']);
});

//Endpoints sedes

$sedesService = new SedesService();

$app->get("/sedes",function() use ($app,$sedesService) {
    $query = $app->request->getQuery();
    unset($query["_url"]);
    $data = $sedesService->listar($query);
    response($app,$data['mensaje'],$data['status']);
});

$app->post("/sedes",function() use ($app,$sedesService) {
    $sedes = json_decode($app->request->getRawBody());
    $data = $sedesService->nuevo($sedes);
    response($app,$data['mensaje'],$data['status']);
});

$app->put("/sedes/{id:[0-9]+}",function($id) use ($app,$sedesService) {
    $sedes = json_decode($app->request->getRawBody());
    $data = $sedesService->modificar($id,$sedes);
    response($app,$data['mensaje'],$data['status']);
});

$app->delete("/sedes/{id:[0-9]+}",function($id) use ($app,$sedesService) {
    $data = $sedesService->eliminar($id);
    response($app,$data['mensaje'],$data['status']);
});
